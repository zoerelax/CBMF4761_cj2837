---
title: "bhr_functional_variant_multiplex"
output:
  html_document:
    toc: true
    toc_float: true
...
# 2. Define a function to solve for beta given RR_adj
...
# 3. Calculate beta, p, 2pq, and h^2
h2_table <- rr_table %>%
  rowwise() %>%
  mutate(
...
save(h2_summary, file = "~/Documents/Yufeng_Lab/BHR/final/multiplex/multiplex_summary.RData")
```
...
ggplot(h2_summary, aes(x = h2, y = bin, fill = grp)) +
  ...
  labs(
    ...
    title = "Partitioned h² (effect size² * frequency) for Multiplex"
  ) +
  theme_minimal()
```
